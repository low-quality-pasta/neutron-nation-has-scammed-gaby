...
#define RadianToDegree( Radian )			( ( Radian ) * ( 180.0f / PI ) )
	...
	BOOLEAN Initialize()
	{
		...
		// GObjects
		auto addr = FindPattern(xorstr("\x48\x8B\x05\x00\x00\x00\x00\x4C\x8D\x3C\xCD"), xorstr("xxx????xxxx"));
		...
		addr = FindPattern(xorstr("\xF3\x0F\x5F\x0D\x00\x00\x00\x00\x41\x8B\x41\x08"), xorstr("xxxx????xxxx"));	
		addr -= 0x280;
